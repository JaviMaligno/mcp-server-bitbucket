...
// Client is used by tools and resources internally
...
async function runStdioServer(): Promise<void> {
  ...
  console.error(`Bitbucket MCP Server v${VERSION} started (stdio mode)`);
}
...
async function runHttpServer(port: number): Promise<void> {
  ...
  httpServer.listen(port, () => {
    console.log(`Bitbucket MCP Server v${VERSION} listening on http://0.0.0.0:${port} (HTTP mode)`);
  });
